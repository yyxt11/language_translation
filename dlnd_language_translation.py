...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for line in source_text.split('\n'):
        source_id_text.append([source_vocab_to_int[vocab] for vocab in line.split()])
    
    for line in target_text.split('\n'):
        target_temp = []
        target_temp = [target_vocab_to_int[vocab] for vocab in line.split()]
        target_temp.append(target_vocab_to_int['<EOS>'])
        target_id_text.append(target_temp)
...
def model_inputs():
    ...
    target_data = tf.placeholder(tf.int32,[None,None],name='output')
    learning_rate = tf.placeholder(tf.float32,name='learn_rate')
    keep_prob = tf.placeholder(tf.float32,name='keep_prob')
    return input_data, target_data, learning_rate, keep_prob
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    # TODO: Implement Function
    #print(batch_size)
    _end_cut = tf.strided_slice(target_data,[0,0],[batch_size,-1],[1,1])
    #print(_end_cut)
    _go_add = tf.fill([batch_size,1],target_vocab_to_int['<GO>'])
    #print(_go_add)
    new_target = tf.concat([_go_add,_end_cut],1)
    return new_target
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    #drop
    drop_cell = tf.contrib.rnn.DropoutWrapper(rnn_cell,keep_prob)
    #rnn layer
    rnn_output,state = tf.nn.dynamic_rnn(drop_cell,rnn_inputs,dtype=tf.float32)
    # TODO: Implement Function
    return state
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    train_pred,_,_ = tf.contrib.seq2seq.dynamic_rnn_decoder(drop_cell,train_decode_fn,
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    """
    # TODO: Implement Function
    inf_decode_fn = tf.contrib.seq2seq.simple_decoder_fn_inference(output_fn,encoder_state,
                                                                   dec_embeddings,start_of_sequence_id,
                                                                  end_of_sequence_id,maximum_length-1,
                                                                  vocab_size)
    #cell drop 
    drop_cell = tf.contrib.rnn.DropoutWrapper(dec_cell,keep_prob)
    
    inference_logits,_,_ = tf.contrib.seq2seq.dynamic_rnn_decoder(drop_cell,inf_decode_fn,scope=decoding_scope)
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    with tf.variable_scope('decoding') as decoding_scope:
        training_logits = decoding_layer_train(encoder_state,drop_cell,
                                               dec_embed_input,sequence_length,
    ...
    with tf.variable_scope("decoding", reuse=True) as decoding_scope:
        inference_logits = decoding_layer_infer(encoder_state,drop_cell,
                                               dec_embeddings,start_of_sequence_id,
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    """
    # TODO: Implement Function
    embed_encode_input = tf.contrib.layers.embed_sequence(input_data,source_vocab_size,enc_embedding_size)
    encoder_state= encoding_layer(embed_encode_input,rnn_size,num_layers,keep_prob)
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    word_id = [vocab_to_int.get(word,vocab_to_int['<UNK>']) for word in lower_sentences.split()]
    return word_id
...
translate_sentence = 'what is your name.'
